(()=>{
  ...
  function runAfterImport(){
    ...
    setTimeout(()=>$('#btnRunAudit')?.click(),180);setTimeout(()=>$('#btnBuildReport')?.click(),520);setTimeout(()=>window.LPS_MULTIOBRA?.render?.(),620);setTimeout(()=>window.LPS_AUTO_PREVIEW?.prepareAll?.(false),760);setTimeout(()=>window.LPS_CURVE_SELECTION?.render?.(),900);setTimeout(()=>window.LPS_CURVE_PREVIEW_V2?.render?.(),980);

...
const state={model:null,exportScope:false,engineWrapped:false,renderTimer:null,aiFindings:[],fileRevision:0};
...
function bumpExportFileVersion(){
 const input=$('#scheduleFile'),f=input?.files?.[0];if(!f||typeof DataTransfer==='undefined'||typeof File==='undefined')return;
 try{const dt=new DataTransfer(),copy=new File([f],f.name,{type:f.type,lastModified:Date.now()+(++state.fileRevision)});dt.items.add(copy);input.files=dt.files}catch(e){console.warn('Não foi possível invalidar o cache de escopo.',e)}
}
...
function install(){ensureUI();const timer=setInterval(()=>{if(wrapEngine())clearInterval(timer)},100);for(const id of['filterSector','filterArea','filterDiscipline'])$("#"+id)?.addEventListener('change',()=>{scheduleRender();bumpExportFileVersion();setTimeout(()=>window.LPS_AUTO_PREVIEW?.prepareAll?.(true),120)});window.addEventListener('lps-ai-ready',e=>{state.aiFindings=e.detail?.analysis?.findings||[];renderSuggestions()});if(window.LPS_LAST_MODEL){state.model=enrichModel(window.LPS_LAST_MODEL);scheduleRender()}}
